...
DBsql.prototype.getModelMultisel = function (jsh, model, lovfield, allfields, sql_foreignkeyfields, datalockqueries, lov_datalockqueries, param_datalocks) {
  ...
  else if (lov.code || lov.code_sys || lov.code_app) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' from ' + _this.getLOVTable(jsh, model, lov) + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + ">CURRENT_TIMESTAMP)"; }
...
DBsql.prototype.postModelForm = function (jsh, model, fields, keys, sql_extfields, sql_extvalues, hashfields, param_datalocks, datalockqueries) {
  ...
  var sql_keys = '';
  _.each(keys, function (field) {
    sql_keys += ' and ' + field.name + '=' + XtoDB(jsh, field, '@' + field.name);
  });

  sql = 'UPDATE ' + tbl + ' SET ' + sql_fields + ' WHERE (%%%SQLWHERE%%%) %%%DATALOCKS%%%' + sql_keys + ';return_row_count()';
...
DBsql.prototype.postModelMultisel = function (jsh, model, lovfield, lovvals, foreignkeyfields, param_datalocks, datalockqueries, lov_datalockqueries) {
  ...
  var lov = lovfield.lov;
  if ('sql' in lov) { lovsql = lov['sql']; }
  else if (lov.code || lov.code_sys || lov.code_app) { lovsql = 'select ' + jsh.map.code_val + ',' + jsh.map.code_txt + ',' + jsh.map.code_seq + ' AS SEQ from ' + _this.getLOVTable(jsh, model, lov) + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + ">CURRENT_TIMESTAMP)"; }
  else throw new Error('LOV type not supported.');
...
DBsql.prototype.getLOV = function (jsh, fname, lov, datalockqueries, param_datalocks, options) {
  ...
  else if (lov.code || lov.code_sys || lov.code_app) { sql = 'select ' + jsh.map.code_val + ' AS "' + jsh.map.code_val + '",' + jsh.map.code_txt + ' AS "' + jsh.map.code_txt + '",' + jsh.map.code_seq + ' from ' + _this.getLOVTable(jsh, options.model, lov) + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + ">CURRENT_TIMESTAMP) order by " + jsh.map.code_seq + ',' + jsh.map.code_txt; }
  else if (lov.code2 || lov.code2_sys || lov.code2_app) { sql = 'select ' + jsh.map.code_val + '1 as "' + jsh.map.code_parent + '",' + jsh.map.code_val + '2 as "' + jsh.map.code_val + '",' + jsh.map.code_txt + ' AS "' + jsh.map.code_txt + '" from ' + _this.getLOVTable(jsh, options.model, lov) + ' where (' + jsh.map.code_end_date + ' is null or ' + jsh.map.code_end_date + ">CURRENT_TIMESTAMP) order by " + jsh.map.code_seq + ',' + jsh.map.code_txt; }
...
DBsql.prototype.getLOVFieldTxt = function (jsh, model, field) {
  ...
  if(lov.values){
    ...
    rslt = 'select ' + jsh.map.code_txt + ' AS "' + jsh.map.code_txt + '" from ' + _this.getLOVTable(jsh, model, lov) + ' where ' + jsh.map.code_val + '=(' + valsql + ')';
  }
  else if (lov.code2 || lov.code2_sys || lov.code2_app) {
    if (!parentsql) throw new Error('Parent field not found in LOV.');
    rslt = 'select ' + jsh.map.code_txt + ' AS "' + jsh.map.code_txt + '" from '+ _this.getLOVTable(jsh, model, lov) + ' where ' + jsh.map.code_val + '1=(' + parentsql + ') and ' + jsh.map.code_val + '2=(' + valsql + ')';
  }
