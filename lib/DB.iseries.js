...
DBdriver.prototype.getConnectionPool = function (dbconfig, onPool) {
  ...
  if(odbcpool.pool) return onPool(null, odbcpool.pool);
...
*/
var ibmErrorPrefix = /^(\[[^\]]+\])*/;
var networkErrorCodes = [10054, 8413, 11001];
DBdriver.prototype.ExecError = function(err, callback, errprefix, sql) {
  ...
  (err.odbcErrors || []).forEach(function(message) {
    if (networkErrorCodes.indexOf(message.code) != -1) closeConnections = true;
  });

  if (closeConnections) {
    if (this.platform.Config.debug_params.db_error_sql_state && !this.silent)     this.platform.Log('Network connection error, resetting database connections', { source: 'database' });
