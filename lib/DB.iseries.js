...
DBdriver.prototype.getCapabilities = function(context){
  return ['recordset','multirecordset','row','command','scalar'];
};
...
DBdriver.prototype.getConnectionPool = function (dbconfig, onPool) {
  ...
  if(odbcpool.pool) {
    resetIdleTimer(odbcpool);
    return onPool(null, odbcpool.pool);
  }
  else if (odbcpool.pending.length > 0){
    odbcpool.pending.push(onPool);
  }
  else { //Initialize pool connection if it was not initialized
...
function closePool(odbcpool, pool_cb) {
  ...
  if(odbcpool.idleTimer){
    clearTimeout(odbcpool.idleTimer);
    odbcpool.idleTimer = null;
  }
...
function resetIdleTimer(odbcpool) {
  if(odbcpool.idleTimer){
    clearTimeout(odbcpool.idleTimer);
    odbcpool.idleTimer = null;
  }
  if(!odbcpool.pool) return;
  var idle_disconnect = odbcpool.dbconfig.options.idle_disconnect;
  if ((typeof(idle_disconnect) == 'number') && (idle_disconnect > 0)) {
    odbcpool.idleTimer = setTimeout(closePool, idle_disconnect, odbcpool);
  }
}
    ...
    var onConnect = function (err, con) {
      ...
      session(null, con, preStatements, function (err) {
        ...
        if (isNetworkError(err)) {
          if (_this.platform.Config.debug_params.db_error_sql_state && !_this.silent) _this.platform.Log('Network connection error, resetting database connection', { source: 'database' });

          return _this.Close(function() {
            if ((networkRetryCount < MAX_NETWORK_RETRY_COUNT) && ((err.statementIndex||0) == 0)) {
...
function isNetworkError(err) {
  if (!err) return false;
  if (!err.odbcErrors || !err.odbcErrors.length) return false;
  for(var i=0; i<err.odbcErrors.length; i++){
    if (networkErrorCodes.indexOf(err.odbcErrors[i].code) >= 0) return true;
  }
  return false;
}
...
DBdriver.prototype.Exec = function (dbtrans, context, return_type, sql, ptypes, params, callback, dbconfig) {
  if(!dbconfig) throw new Error('dbconfig is required');
  if(!_.includes(['recordset','multirecordset','row','command','scalar'], return_type)) throw new Error('DB operation not supported: '+(return_type||'').toString());
  var _this = this;
